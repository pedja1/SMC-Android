...
public class LevelEditorScreen implements Screen, InputProcessor {
    ...
    public LevelEditorScreen(java.util.List<GameObject> gameObjects, FileHandle file) {
    ...
    public void show() {
        ...
        assets.load("data/dynamic.pack", TextureAtlas.class);
        assets.load("data/static.pack", TextureAtlas.class);
        assets.finishLoading();
        dynamicAtlas = assets.get("data/dynamic.pack");
        staticAtlas = assets.get("data/static.pack");
        ...
        Array<TextureAtlas.AtlasRegion> allRegions = new Array<>();
        allRegions.addAll(dynamicAtlas.getRegions());
        allRegions.addAll(staticAtlas.getRegions());
    ...
    public boolean keyDown(int keycode) {
        ...
        if (keycode == Input.Keys.A) {
            ...
            gameObjects.remove(draggingObject);
            draggingObject = null;
    ...
    public boolean touchDown(int x, int y, int pointer, int button) {
        ...
        for (GameObject go : gameObjects) {
            ...
            if (verticalResizeHover || horizontalResizeHover) {
                bounds = tmp1;
                bounds.set(go.drawRect.x - RESIZE_OFFSET, go.drawRect.y - RESIZE_OFFSET, go.drawRect.width + RESIZE_OFFSET * 2, go.drawRect.height + RESIZE_OFFSET * 2);
            } else {
                bounds = go.drawRect;
            }
            if (bounds.contains(myX, myY)) {
                found = true;
                draggingObject = go;
                downDistanceX = myX - go.drawRect.x;
                downDistanceY = myY - go.drawRect.y;
                break;
            }
        }
        if (!found) {
    ...
    private void addObject() {
        ...
        if (selected.startsWith("tiles")) {
            TextureRegion region = staticAtlas.findRegion(textureName);
            Sprite sprite = new Sprite(3, 3, 1, 1, (float) region.getRegionHeight() / (float) region.getRegionWidth(), null);
            sprite.textureAtlas = Assets.ATLAS_STATIC;
            ...
            TextureRegion tileRegion = staticAtlas.findRegion("environment/tiles/1");
            ...
            Sprite sprite = new Sprite(3, 3, 0, width, width / ((float) region.getRegionWidth() / (float) region.getRegionHeight()), null);
            sprite.textureAtlas = Assets.ATLAS_STATIC;
            sprite.textureName = textureName;
            ...
            Coin coin = new Coin(3, 3, 0, Coin.DEF_SIZE, Coin.DEF_SIZE, selected.contains("red") ? Coin.TYPE_RED : Coin.TYPE_YELLOW);
            coin.initAssets();
    ...
    private void snapToGrid() {
        if (draggingObject == null || dragging)
            return;
        //snap to grid
        int pX = Math.round(draggingObject.drawRect.x);
        int pY = Math.round(draggingObject.drawRect.y);
        draggingObject.position.set(pX, pY, draggingObject.position.y);
    }
    ...
    public boolean touchDragged(int x, int y, int pointer) {
        ...
        if (draggingObject != null) {
            ...
            if (horizontalResizeHover) {
                float myX = x / (width / cam.viewportWidth);
                boolean isLeft = Math.abs(myX - draggingObject.drawRect.x) < Math.abs(myX - (draggingObject.drawRect.x + draggingObject.drawRect.width));
                if (isLeft) {
                    //must change both position and width
                    if (myX < draggingObject.drawRect.x + draggingObject.drawRect.width) {
                        float diff = draggingObject.drawRect.x - myX;
                        draggingObject.drawRect.width += diff;
                        draggingObject.position.x = myX;
                    }
                } else {
                    //set width
                    if (myX > draggingObject.drawRect.x)
                        draggingObject.drawRect.width = myX - draggingObject.drawRect.x;
                }
            } else if (verticalResizeHover) {
                float myY = invertY(y) / (height / cam.viewportHeight);

                boolean isBottom = Math.abs(myY - draggingObject.drawRect.y) < Math.abs(myY - (draggingObject.drawRect.y + draggingObject.drawRect.height));
                if (isBottom) {
                    //must change both position and width
                    if (myY < draggingObject.drawRect.y + draggingObject.drawRect.height) {
                        float diff = draggingObject.drawRect.y - myY;
                        draggingObject.drawRect.height += diff;
                        draggingObject.position.y = myY;
                    }
                } else {
                    //set width
                    if (myY > draggingObject.drawRect.y)
                        draggingObject.drawRect.height = myY - draggingObject.drawRect.y;
                }
            } else {
                float myY = invertY(y) / (height / cam.viewportHeight) - downDistanceY;
                float myX = x / (width / cam.viewportWidth) - downDistanceX;
                dragging = true;
                draggingObject.position.set(myX, myY, draggingObject.position.z);
            }
        }
        return false;
    }
    ...
    public boolean mouseMoved(int screenX, int screenY) {
        ...
        for (GameObject gameObject : gameObjects) {
            tmp1.set(gameObject.drawRect.x - RESIZE_OFFSET, gameObject.drawRect.y - RESIZE_OFFSET,
                    gameObject.drawRect.width + RESIZE_OFFSET * 2, gameObject.drawRect.height + RESIZE_OFFSET * 2);

            if (!tmp1.contains(x, y))
                continue;

            tmp2.set(gameObject.drawRect.x + RESIZE_OFFSET, gameObject.drawRect.y + RESIZE_OFFSET,
                    gameObject.drawRect.width - RESIZE_OFFSET * 2, gameObject.drawRect.height - RESIZE_OFFSET * 2);
